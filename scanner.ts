...
export class Token {
    kind: TokenKind;
    code: Op|Seperator|Keyword|null; // token的code 有分隔符，运算符和关键字三种
    text: string;
    pos: Position;
...
export class Position {
    begin:number; // Token起始字符位置
    end:number; // Token结束字符位置
    line:number; // 所在行号，从1开始
    col:number; //所在列号，从1开始
...
export class CharStream {
    ...
    getPosition():Position {
        return new Position(this.pos+1, this.pos+1, this.line, this.col); // 计算Token的position
    }
